...
public class PlayerMovement : MonoBehaviour {
    ...
    void Update() {
        ...
        float horizontalVelocity = inputValue.x;
        float verticalVelocity = inputValue.y;

        // Rotation
        if (horizontalVelocity != 0) {
            playerRigidbody.transform.eulerAngles = new Vector3(0, horizontalVelocity > 0 ? 180 : 0, 0); // Rotate 180 degrees on the Y-axis
        }
        ...
        if (isSwimming) {
            ...
            if (isTouchingWater) {
                horizontalVelocity *= (walkspeed + swimspeed) / 2;
            } else {
    ...
    private void CheckEnvironment() {
        ...
        Vector2 playerBox = new Vector2(playerCollider.bounds.size.x, playerCollider.bounds.size.y);

        isGrounded = Physics2D.Raycast(playerFeet, Vector2.down, 0.1f, groundLayer) && Mathf.Approximately(playerRigidbody.linearVelocity.y, 0);
    ...
    public void ToggleFlight() {
        canFly = !canFly;
        Physics2D.gravity = canFly ? Vector2.zero : new Vector3(0f, -9.81f, 0f);
    }
